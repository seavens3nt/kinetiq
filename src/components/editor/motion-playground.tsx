...
export function MotionPlayground() {
      ...
      useEditorStore.setState((state) => {
        ...
        const component: VisualComponent = type === "image"
          ? {
              ...
              width: Math.round(state.project.width * 0.7), height: Math.round(state.project.height * 0.5),
  ...
  const transitionStyle = (component: VisualComponent) => {
    ...
    <main className="flex h-screen min-h-0 flex-col overflow-hidden bg-[#0B0B0F] text-[#F7F7F4]">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-[#101015] px-4 lg:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="rounded-full border border-[#8067FF]/25 bg-[#8067FF]/10 px-3 py-1.5 text-[11px] text-[#C7BEFF]">{timeline.layers.length} layers · {allComponents.length} components{timeline.musicTracks.length ? ` · ${timeline.musicTracks.length} music` : ""}</div>
        </div>
      </header>
          ...
          >
            ...
            <div className={`relative min-h-0 overflow-hidden rounded-[22px] border border-white/10 shadow-2xl ${background.className} ${isLandscape ? "w-[min(100%,900px)] max-h-full" : "h-full max-w-full"}`} style={{ aspectRatio: `${project.width} / ${project.height}` }}>
              ...
              {visibleComponents.map(({ component, layerId }, index) => {
                ...
                const shared = { left, top, width, height, ...transition };
                const select = () => setSelectedComponent(component.id, layerId);

                if (component.type === "text") {
                  return <button key={component.id} onClick={select} className={`absolute flex items-center justify-center rounded-xl px-2 outline-none ${isSelected ? "ring-2 ring-[#D7FF45]/70" : ""}`} style={shared}><AnimatedText text={component.content} presetId={isSelected && previewPresetId ? previewPresetId : component.motionPresetId} motionSettings={component.motionSettings} replayKey={activeReplayKey + index + Math.round(component.startTime * 100)} /></button>;
                }
                if (component.type === "image" && component.src) {
                  return <button key={component.id} onClick={select} className={`absolute overflow-hidden rounded-xl ${isSelected ? "ring-2 ring-[#D7FF45]/70" : ""}`} style={shared}><img src={component.src} alt={component.name} className={`h-full w-full ${component.fit === "cover" ? "object-cover" : "object-contain"}`} /></button>;
                }
                if (component.type === "video" && component.src) {
                  return <button key={component.id} onClick={select} className={`absolute overflow-hidden rounded-xl bg-black ${isSelected ? "ring-2 ring-[#D7FF45]/70" : ""}`} style={shared}><video src={component.src} muted={component.muted} autoPlay loop playsInline defaultPlaybackRate={component.playbackRate ?? 1} className="h-full w-full object-contain" /></button>;
                }
