...
export function AuthProjectControls() {
  ...
  return (
    <>
      ...
      {authOpen && <div className="fixed inset-0 z-[100] grid place-items-center bg-black/65 p-4 backdrop-blur-sm"><div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#121218] p-5 shadow-2xl">
        ...
        <div className="space-y-3"><input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="Email" className="w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2.5 text-xs outline-none focus:border-[#D7FF45]/60" /><input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Password" className="w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2.5 text-xs outline-none focus:border-[#D7FF45]/60" />{message && <div className={`text-[10px] leading-4 ${message.includes("sent") || message.includes("confirm") ? "text-[#D7FF45]" : "text-red-300"}`}>{message}</div>}<button onClick={submitEmailAuth} disabled={!email || password.length < 6} className="w-full rounded-lg bg-[#D7FF45] px-3 py-2.5 text-xs font-bold text-[#0B0B0F] disabled:opacity-40">{mode === "signin" ? "Sign in" : "Create account"}</button></div>
